...
const Alert: React.FC<AlertProps> = ({ title, message, type }) => {
  ...
  const { border, bg, iconBg, titleColor, messageColor, svg } =
    alertStyles[type];

  return (
    <div
      className={`flex w-full border-l-6 ${border} ${bg} px-7 py-4 shadow-md dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-4.5`}
    >
      <div
      ...
      <div className="w-full">
        <h5 className={`mb-3 text-lg font-semibold ${titleColor}`}>{title}</h5>
        <p className={`leading-relaxed ${messageColor}`}>{message}</p>
      </div>
    </div>
  );
};
